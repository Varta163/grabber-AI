...
@router.post("/fetch")
def trigger_fetch(db: Session = Depends(get_db), _: User = Depends(require_admin)):
...
@router.post("/process")
def trigger_process(batch_size: int = 20, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    return process_pending(db, batch_size=batch_size)


@router.post("/trends")
def trigger_trends(hours: int = 24, db: Session = Depends(get_db), _: User = Depends(require_admin)):
    trends = run_trend_detection(db, hours=hours)
    return {"trends_detected": len(trends), "trends": trends}


@router.post("/brief")
def trigger_brief(db: Session = Depends(get_db), _: User = Depends(require_admin)):
...
@router.post("/run-all")
def run_full_pipeline(background_tasks: BackgroundTasks, _: User = Depends(require_admin)):
